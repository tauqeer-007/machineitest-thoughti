exports.recordFetch = function (req, res) {

    let pageSize = req.query.page_size ? req.query.page_size : 10
    let pageNumber = req.query.page_number ? req.query.page_number : 1

    let record = getRecords();
    let paginatedData = paginate(record, pageSize, pageNumber)
    let ids = [];
    let open = [];
    let closedCount = 0;
    paginatedData.map((dataObj) => {
        ids.push(dataObj.id)
        if (dataObj.disposition == "open") {
            open.push(dataObj)
        }
        if (dataObj.disposition == "closed") {
            closedCount++
        }
    })
    let paginationLinkData = paginationLink(record, pageSize, pageNumber)
...
function paginationLink(array, page_size, page_number) {
    let dataCount = array.slice((page_number - 1) * page_size, page_number * page_size);
    let prevLink = null;
    if ((page_number - 1) != 0 && page_number > 1) {
        let num = page_number - 1
        prevLink = '/records?page_number=' + num + '&page_size=' + page_size
    }
    let nextLink = null;
    if (dataCount.length > 0) {
        let num = parseInt(page_number) + 1
        nextLink = '/records?page_number=' + num + '&page_size=' + page_size
    }
    let links = {
        prevLink,
        nextLink
